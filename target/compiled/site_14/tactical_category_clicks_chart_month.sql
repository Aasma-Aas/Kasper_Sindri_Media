...
CREATE DEFINER=`Site`@`%` PROCEDURE `prod`.`tactical_category_clicks_chart_month_dbt_14`()
BEGIN
...
where g.date  between DATE_SUB(NOW(), INTERVAL 31 DAY) and DATE_SUB(NOW(), INTERVAL 1 DAY)  and g.site_id =  14
),
data_out as
(
  select 
