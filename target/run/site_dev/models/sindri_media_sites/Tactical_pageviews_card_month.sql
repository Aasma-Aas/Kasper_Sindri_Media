...
CREATE DEFINER=`Site`@`%`  PROCEDURE `prod`.`Tactical_pageviews_card_month_dbt_17`()
BEGIN
...
where date  between DATE_SUB(NOW(), INTERVAL 60 DAY) and DATE_SUB(NOW(), INTERVAL 30 DAY) and e.siteid = 17
group by 1,agg
),
json_data as( 
