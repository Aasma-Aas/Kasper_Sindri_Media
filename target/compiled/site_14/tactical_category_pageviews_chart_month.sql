...
CREATE DEFINER=`Site`@`%` PROCEDURE `prod`.`tactical_category_pageviews_chart_month_dbt_14`()
BEGIN
    ...
    less_tg_data as(
        ...
        where date
		between DATE_SUB(NOW(), INTERVAL 31 DAY) and DATE_SUB(NOW(), INTERVAL 1 DAY)  and a.siteid = 14 
    ),
counts as (
        select 
            siteid,
