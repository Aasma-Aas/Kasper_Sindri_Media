...
CREATE PROCEDURE `prod`.`tactical_userneeds_pageviews_chart_ytd_dbt_14`()
BEGIN
	...
	less_tg_data as(
		...
		join prod.goals g on a.siteid = g.site_id and g.Date = l.fdate
		 where date between MAKEDATE(EXTRACT(YEAR FROM CURDATE()),1)  and  DATE_SUB(cast(NOW() as date), INTERVAL 1 DAY) and g.site_id = 14
		),
	counts as (
			select 
				siteid,
	...
	counts_second as (
		...
		categories_d_second as (
			...
			'Artikler ift. next click mål' as label,
            'help' as hint,
			GROUP_CONCAT(tags ORDER BY FIELD(tags, 'Nyhed','Debat','Guide','Artikel','Digidoc','post','news_overview','','','', 'others' ) SEPARATOR ',') 
			AS cateogires,
			GROUP_CONCAT(COALESCE(less_than_target, 0) ORDER BY FIELD(tags, 'Nyhed','Debat','Guide','Artikel','Digidoc','post','news_overview','','','', 'others' ) SEPARATOR ',') 
			AS less_than_target,
			GROUP_CONCAT(approved ORDER BY FIELD(tags, 'Nyhed','Debat','Guide','Artikel','Digidoc','post','news_overview','','','', 'others' ) SEPARATOR ',') 
			AS approved,
			GROUP_CONCAT(top_10 ORDER BY FIELD(tags, 'Nyhed','Debat','Guide','Artikel','Digidoc','post','news_overview','','','', 'others' ) SEPARATOR ',') 
			AS top_10
	...
	counts_third as (
		...
		categories_d_third as (
			...
			'Artikler ift. next click mål' as label,
            'help' as hint,
			GROUP_CONCAT(tags ORDER BY FIELD(tags, 'Arbejdsmiljø','Social dumping','Ulighed','Privatøkonomi', 'others' ) SEPARATOR ',') 
			AS cateogires,
			GROUP_CONCAT(COALESCE(less_than_target, 0) ORDER BY FIELD(tags, 'Arbejdsmiljø','Social dumping','Ulighed','Privatøkonomi', 'others' ) SEPARATOR ',') 
			AS less_than_target,
			GROUP_CONCAT(approved ORDER BY FIELD(tags, 'Arbejdsmiljø','Social dumping','Ulighed','Privatøkonomi', 'others' ) SEPARATOR ',') 
			AS approved,
			GROUP_CONCAT(top_10 ORDER BY FIELD(tags, 'Arbejdsmiljø','Social dumping','Ulighed','Privatøkonomi', 'others' ) SEPARATOR ',') 
			AS top_10
		from agg_data_third
		group by 1,2,3
		),
