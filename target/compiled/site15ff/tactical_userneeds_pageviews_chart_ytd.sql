...
CREATE PROCEDURE `prod`.`tactical_userneeds_pageviews_chart_ytd_dbt_15`()
BEGIN

    with 
	 
	last_ytd_days_article_published as(
	 ...
	 last_ytd_days_article_published_Agg as(
		select 
            ...
            tags,
            count(*) as agg 
		from  last_ytd_days_article_published
		where siteid = 15
		group by 1,2
		),
      ...
      (
		...
		from last_ytd_days_article_published  e
	...
	less_tg_data as(
		...
		join prod.goals g on a.siteid = g.site_id and g.Date = l.fdate
		 where date between MAKEDATE(EXTRACT(YEAR FROM CURDATE()),1)  and  DATE_SUB(cast(NOW() as date), INTERVAL 1 DAY) and g.site_id = 15
		),
	counts as (
			select 
				siteid,
		...
		agg_data as(
			...
			join last_ytd_days_article_published_Agg hbt on hbt.siteid=h.siteid and hbt.tags=h.tags
			where h.siteid  = 15
		),
		categories_d as (
			select 
	...
	last_ytd_days_article_published_second as(
		SELECT siteid as siteid,id,  date as fdate,
		CASE

			 

			WHEN Categories REGEXP  ".*Leder.*" THEN "Leder" 
			
			 

			WHEN Categories REGEXP  ".*Presse.*" THEN "Presse" 
			
			 

			WHEN Categories REGEXP  ".*Privatansat.*" THEN "Privatansat" 
			
			 

			WHEN Categories REGEXP  ".*Plejefamilie.*" THEN "Plejefamilie" 
			
			 

			WHEN Categories REGEXP  ".*Tillidsrepræsentant.*" THEN "Tillidsrepræsentant" 
			
			 

			WHEN Categories REGEXP  ".*Lærere.*" THEN "Lærere" 
			
			 

			WHEN Categories REGEXP  ".*Arbejdsmiljørepræsentant.*" THEN "Arbejdsmiljørepræsentant" 
			
			 

			WHEN Categories REGEXP  ".*TRIO.*" THEN "TRIO" 
	 ...
	 last_ytd_days_article_published_Agg_second as(
		select 
			siteid as siteid,
            tags,
            count(*) as agg 
		from  last_ytd_days_article_published_second
		where siteid = 15
		group by 1,2
		),
      ...
      (
		select  
            ...
            e.tags,
            coalesce(sum(hits),0) as val
		from last_ytd_days_article_published_second  e
		left join prod.events  a on a.postid=e.id and a.siteid = e.siteid 
		and a.event_action = 'Next Click'
		where    e.siteid = 15 
		group by 1,2,3
	...
	less_tg_data_second as(
		...
		case when val<min_cta then 1 else 0 end as less_than_target
		,percent_rank() over ( order BY case when val>=min_cta then val-min_cta else 0 end) as percentile
		 from last_ytd_days_article_published_second l
	...
	counts_second as (
		...
		agg_data_second as(
			...
			join last_ytd_days_article_published_Agg_second hbt on hbt.siteid=h.siteid and hbt.tags=h.tags
			where h.siteid  = 15
		),
		categories_d_second as (
			...
			'Artikler ift. next click mål' as label,
            'help' as hint,
			GROUP_CONCAT(tags ORDER BY FIELD(tags, 'Leder','Presse','Privatansat','Plejefamilie','Tillidsrepræsentant','Lærere','Arbejdsmiljørepræsentant','TRIO','','', 'others' ) SEPARATOR ',') 
			AS cateogires,
			GROUP_CONCAT(COALESCE(less_than_target, 0) ORDER BY FIELD(tags, 'Leder','Presse','Privatansat','Plejefamilie','Tillidsrepræsentant','Lærere','Arbejdsmiljørepræsentant','TRIO','','', 'others' ) SEPARATOR ',') 
			AS less_than_target,
			GROUP_CONCAT(approved ORDER BY FIELD(tags, 'Leder','Presse','Privatansat','Plejefamilie','Tillidsrepræsentant','Lærere','Arbejdsmiljørepræsentant','TRIO','','', 'others' ) SEPARATOR ',') 
			AS approved,
			GROUP_CONCAT(top_10 ORDER BY FIELD(tags, 'Leder','Presse','Privatansat','Plejefamilie','Tillidsrepræsentant','Lærere','Arbejdsmiljørepræsentant','TRIO','','', 'others' ) SEPARATOR ',') 
			AS top_10
	...
	last_ytd_days_article_published_third as(
		SELECT siteid as siteid,id,  date as fdate,
		CASE
			...
			WHEN Tags REGEXP  ".*Autisme|Udviklingshandicap|Botilbud|Magtanvendelse|Seksualitet|Domfældte.*" THEN "Handicap" 
			
			 

			WHEN Tags REGEXP  ".*Psykiatri|Misbrug|Væresteder|Hjemløse|Herberg og forsorgshjem|Marginaliserede.*" THEN "Psykiatri og udsathed" 
			...
			WHEN Tags REGEXP  ".*Ansættelsesvilkår|Opsigelse|Overenskomst|Arbejdstid|Løn|Ferie|Barsel|Senior |Ligestilling|Job og karriere|Uddannelse|Meritpædagoguddannelse|Kompetenceudvikling |Efteruddannelse|Arbejdsmiljø|Stress|Sygdom|Vold og trusler|Arbejdsskade|PTSD|A-kasse |Lønforsikring|Ledighed|Efterløn|Dagpenge|Arbejdsløshed.*" THEN "Arbejdsliv og vilkår" 
	 ...
	 last_ytd_days_article_published_Agg_third as(
		select 
			siteid as siteid,
            tags,
            count(*) as agg 
		from  last_ytd_days_article_published_third
		where siteid = 15
		group by 1,2
		),
      ...
      (
		select  
            ...
            e.tags,
            coalesce(sum(hits),0) as val
		from last_ytd_days_article_published_third  e
		left join prod.events  a on a.postid=e.id and a.siteid = e.siteid 
		and a.event_action = 'Next Click'
		where    e.siteid = 15 
		group by 1,2,3
	...
	less_tg_data_third as(
		...
		case when val<min_cta then 1 else 0 end as less_than_target
		,percent_rank() over ( order BY case when val>=min_cta then val-min_cta else 0 end) as percentile
		 from last_ytd_days_article_published_third l
	...
	counts_third as (
		...
		agg_data_third as(
			...
			join last_ytd_days_article_published_Agg_third hbt on hbt.siteid=h.siteid and hbt.tags=h.tags
			where h.siteid  = 15
		),
