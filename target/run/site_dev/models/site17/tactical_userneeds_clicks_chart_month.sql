  ...
  create view `site_dev`.`tactical_userneeds_clicks_chart_month__dbt_tmp`
...
CREATE DEFINER=`Site`@`%` PROCEDURE `prod`.`tactical_userneeds_clicks_chart_month_dbt_15`()
BEGIN
...
,l.siteid from last_ytd_article_published l
 ...
 where g.date
between date_sub(current_date(),interval 1 month)  and  DATE_SUB(cast(NOW() as date), INTERVAL 1 DAY)  and g.site_id = 15
),
data_out as
(
select 
