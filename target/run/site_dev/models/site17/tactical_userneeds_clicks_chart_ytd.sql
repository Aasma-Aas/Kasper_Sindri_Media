  ...
  create view `site_dev`.`tactical_userneeds_clicks_chart_ytd__dbt_tmp`
...
CREATE DEFINER=`Site`@`%` PROCEDURE `prod`.`tactical_userneeds_clicks_chart_ytd_dbt_15`()
BEGIN
...
,l.siteid from last_ytd_article_published l
 ...
 where g.date between MAKEDATE(EXTRACT(YEAR FROM CURDATE()),1)  and  DATE_SUB(cast(NOW() as date), INTERVAL 1 DAY)  and g.site_id = 15
),
data_out as
(
select 
...
,',{"name": "Gns. next click 000.", "data": [',cast(less_than_target_click as char),',',cast(approved_clicks as char),',',cast(top_10_clicks as char),']}') as series from data_out
)
    SELECT 
