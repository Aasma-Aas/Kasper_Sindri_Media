...
CREATE PROCEDURE `prod`.`tactical_userneeds_pageviews_chart_ytd_dbt_17`()
BEGIN
	...
	less_tg_data as(
		...
		join prod.goals g on a.siteid = g.site_id and g.Date = l.fdate
		 where date between MAKEDATE(EXTRACT(YEAR FROM CURDATE()),1)  and  DATE_SUB(cast(NOW() as date), INTERVAL 1 DAY) and g.site_id = 17
		),
	counts as (
			select 
				siteid,
