  ...
  create view `site_dev`.`tactical_category_clicks_chart_month__dbt_tmp`
...
CREATE DEFINER=`Site`@`%` PROCEDURE `prod`.`tactical_category_clicks_chart_month_dbt_15`()
BEGIN
...
where g.date  between DATE_SUB(NOW(), INTERVAL 31 DAY) and DATE_SUB(NOW(), INTERVAL 1 DAY)  and g.site_id =  15
),
data_out as
(
  select 
