...
CREATE {{db_name}} PROCEDURE `prod`.`tactical_category_pageviews_chart_ytd_dbt_{{site}}`()
BEGIN
    ...
    last_30_days_article_published{{ count[i] }} as(
        SELECT siteid as siteid,id,  date as fdate,
            ...
            WHEN {{ chart_count[i] }}  {{tendency_condition[i]}} {% if tendency_condition[i] == 'like' %}  "%{{ chart_val[i][j] }}%" {% else %} "{{ chart_val[i][j] }}" {% endif %} THEN "{{ val_like[i][j] }}"
...
between MAKEDATE(EXTRACT(YEAR FROM CURDATE()),1)  and  DATE_SUB(cast(NOW() as date), INTERVAL 1 DAY)
    ...
    less_tg_data{{ count[i] }} as(
        ...
        where date
between MAKEDATE(EXTRACT(YEAR FROM CURDATE()),1)  and  DATE_SUB(cast(NOW() as date), INTERVAL 1 DAY)  and a.siteid = {{site}} 
    ),
counts{{ count[i] }} as (
        select 
            siteid,
