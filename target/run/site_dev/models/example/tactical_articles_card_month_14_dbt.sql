  ...
  create view `site_dev`.`tactical_articles_card_month_14_dbt__dbt_tmp`
...
CREATE PROCEDURE `prod`.`tactical_articles_card_month_14_dbt`()
BEGIN
with last_30_day as
(
...
where 
date between DATE_SUB(NOW(), INTERVAL 60 DAY) and DATE_SUB(NOW(), INTERVAL 30 DAY) and siteid = 14
