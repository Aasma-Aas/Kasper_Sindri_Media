...
CREATE DEFINER=`Site`@`%` PROCEDURE `prod`.`tactical_category_clicks_chart_ytd_dbt_17`()
BEGIN
...
where g.date  between MAKEDATE(EXTRACT(YEAR FROM CURDATE()),1)  and  DATE_SUB(cast(NOW() as date), INTERVAL 1 DAY)   and g.site_id =  17
),
data_out as
(
  select 
