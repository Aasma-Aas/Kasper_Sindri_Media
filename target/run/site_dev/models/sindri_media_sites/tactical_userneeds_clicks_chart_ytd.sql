...
CREATE DEFINER=`Site`@`%` PROCEDURE `prod`.`tactical_userneeds_clicks_chart_ytd_dbt_14`()
BEGIN
...
,l.siteid from last_ytd_article_published l
 ...
 where g.date between MAKEDATE(EXTRACT(YEAR FROM CURDATE()),1)  and  DATE_SUB(cast(NOW() as date), INTERVAL 1 DAY)  and g.site_id = 14
),
data_out as
(
select 
