...
CREATE DEFINER=`Site`@`%` PROCEDURE `prod`.`tactical_category_pageviews_chart_ytd_dbt_14`()
BEGIN
...
between MAKEDATE(EXTRACT(YEAR FROM CURDATE()),1)  and  DATE_SUB(cast(NOW() as date), INTERVAL 1 DAY)
    ...
    less_tg_data as(
        ...
        where date
between MAKEDATE(EXTRACT(YEAR FROM CURDATE()),1)  and  DATE_SUB(cast(NOW() as date), INTERVAL 1 DAY)  and a.siteid = 14 
    ),
counts as (
        select 
            siteid,
