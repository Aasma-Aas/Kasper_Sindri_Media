  ...
  create view `site_dev`.`tactical_articles_card_ytd__dbt_tmp`
...
CREATE DEFINER=`Site`@`%` PROCEDURE `prod`.`tactical_articles_card_ytd_dbt_15`()
BEGIN
...
json_data as( 
select ld.siteid as siteid,'Artikler' as label,'Artikler publiceret seneste 30 dage ift. forrige 30 dage' as hint,ld.value as valu,COALESCE(((ld.value-lb.value)/lb.value)*100,0)  as chang,'' as progressCurrent
